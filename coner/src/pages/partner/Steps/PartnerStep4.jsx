...
import { db } from "../../../lib/firebase"; // 또는 firebase 경로
import StepHeader from "../../../components/common/Header/StepHeader";
import TextField from "../../../components/ui/formControls/TextField";
import StepSegmentedToggle from "../../../components/ui/StepSegementedToggle";
import Button from "../../../components/ui/Button";
...
const PartnerStep4 = () => {
  ...
  const [partnerAreas, setPartnerAreas] = useState(null);
    ...
    const fetchPartnerData = async () => {
      ...
      try {
        ...
        if (partnerSnap.exists()) {
          ...
          setPartnerData(data);
          setPartnerAreas(data.area || null);
        }
      } catch (error) {}
    };
  ...
  const getAddressLabel = () => {
    if (partnerAreas && partnerAreas.length > 0) {
      return `주소 (${
        partnerData?.company_name || "선택한 업체"
      }의 서비스 지역으로 제한됩니다)`;
    }
    return "주소 (서울 지역으로만 제한되어 있습니다)";
  };
